...
\only<6>{$$v(\rho)=\widetilde{w} \cdot ln(\frac{1}{|\rho|}) + c$$}
\only<7->{$$v(\rho)=\widetilde{w} \cdot ln(\frac{\rho_{max}}{|\rho|})$$}
\only<6->{$$v(\rho_{max})\stackrel{!}{=}0 $$}
\only<6->{$$\Rightarrow c=-\widetilde{w} \cdot ln(|\frac{\rho_{max}}{\rho}|)$$}
\end{frame}
